...
public class UserDetailsServiceImpl implements UserDetailsService  {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        if(adminRepository.existsByEmail(email)){
            AdminEntity adminEntity = adminRepository.findByEmail(email).get();
            password = adminEntity.getPassword();
            authoritys = getAuthoritiesAdmin(adminEntity);
        }else if(workerRepository.existsByEmail(email)){
            WorkerEntity workerEntity = workerRepository.findByEmail(email).get();
    ...
    private List<SimpleGrantedAuthority> getAuthoritiesAdmin (AdminEntity adminEntity){
        List<SimpleGrantedAuthority> authorities = new ArrayList<>();
        authorities.add(new SimpleGrantedAuthority("ROLE_" + adminEntity.getRole()));
        return authorities;
    }
    private List<SimpleGrantedAuthority> getAuthoritiesCustomer (CustomerEntity customerEntity){
        List<SimpleGrantedAuthority> authorities = new ArrayList<>();
        authorities.add(new SimpleGrantedAuthority("ROLE_" + customerEntity.getRole()));
        return authorities;
    }
}